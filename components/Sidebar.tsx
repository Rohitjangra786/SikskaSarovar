  ...
  return (
    <div className="w-72 bg-white h-full border-r border-slate-200 flex flex-col overflow-hidden shadow-sm">
      <div className="p-8 pb-4 flex items-center gap-3">
        <div className="bg-brand-900 p-2 rounded-xl shadow-lg shadow-brand-900/40 border border-white/10">
          <img src="https://raw.githubusercontent.com/stackblitz/stackblitz-images/main/siksha-logo.png" className="w-8 h-8 object-contain filter brightness-0 invert" alt="Logo" />
        </div>
        <div>
              ...
              >
                <div className="flex items-center gap-3">
                  <div className={`${expandedCourses.includes(course.id) ? 'text-brand-600' : 'text-slate-400 group-hover:text-brand-900'} transition-colors`}>
                    {course.icon === 'layout' ? <Code2 size={18} /> : <Code2 size={18} />}
                  </div>
                  ...
                  {course.lessons.map(lesson => (
                    ...
                    >
                      <div className="flex items-center gap-2">
                        <Circle size={4} className={`fill-current ${lesson.isCompleted ? 'text-brand-600' : 'text-slate-300'}`} />
                        <span className={lesson.isCompleted ? 'font-medium' : ''}>{lesson.title}</span>
                      </div>
                      {lesson.isCompleted && <CheckCircle size={12} className="text-brand-600" />}
                    </button>
                  ))}
                </div>
              )}
      ...
      <div className="p-6 border-t border-slate-100">
        <div className="bg-brand-900 rounded-[2rem] p-5 text-white shadow-xl">
          <div className="flex items-center gap-2 mb-4">
            ...
            <span className="text-[10px] font-black uppercase tracking-wider text-accent-100">Daily Mission</span>
          </div>
          <div className="flex justify-between text-[11px] mb-2 font-bold">
            <span className="opacity-50">XP PROGRESS</span>
            <span className="text-accent-500">450 / 1k</span>
          </div>
          <div className="h-1.5 w-full bg-white/5 rounded-full overflow-hidden mb-1">
            <div className="h-full bg-accent-500 shadow-[0_0_8px_rgba(211,177,109,0.5)]" style={{ width: '45%' }}></div>
          </div>
        </div>
      </div>
    </div>
